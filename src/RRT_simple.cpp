...
class  RRT{
    /*Parameters 
     * start: Start Position Node
     * end: goal location Node
    ...
    public:
        ...
        RRT(cv::Point2i start, cv::Point2i goal, int expand_dist=100,
            ...
            :start(Node(start)), end(Node(goal)), expand_dist(expand_dist),
        ...
        Node * get_random_node(){
            ...
            if(std::rand() % 100 > goal_sample_rate)
                rnd = new Node(std::rand() % max_rand,
                        std::rand() % max_rand);
            else
                rnd = new Node(end);
        ...
        Node * steer(Node *from_node, Node *to_node){
            Node *new_node;
            double dist, theta;
            new_node = new Node(from_node->loc);
        ...
        bool check_collision(Node *from_node, Node *to_node){
        ...
        std::vector<cv::Point> generate_final_course(Node *cur_node){
        ...
        std::vector<Node*> planning(bool show_animation=true){

            std::vector<Node *> path;
            Node *rnd_node, *nearest_node, *new_node, *final_node;
